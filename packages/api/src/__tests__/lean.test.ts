...
beforeAll(async () => {
  ...
  // --- REGISTRO Y LOGIN DEL OWNER ---
  await ownerAgent
    .post('/api/auth/register')
    .send({ email: 'owner@test.com', name: 'Owner User', password: 'password123' });
  ...
  const ownerProfileResponse = await ownerAgent.get('/api/profile');
  ownerId = ownerProfileResponse.body.id;
  expect(ownerId).toBeDefined();
  
  // --- REGISTRO Y LOGIN DEL BORROWER ---
  await borrowerAgent
    .post('/api/auth/register')
    .send({ email: 'borrower@test.com', name: 'Borrower User', password: 'password123' });

  await borrowerAgent
    .post('/api/auth/login')
    .send({ email: 'borrower@test.com', password: 'password123' })
    .expect(200);

  const borrowerProfileResponse = await borrowerAgent.get('/api/profile');
  borrowerId = borrowerProfileResponse.body.id;
  expect(borrowerId).toBeDefined();

  const bookCreateResponse = await ownerAgent
    .post('/api/books')
    .send({ title: 'The Hobbit', published_year: 1937 });
  bookId = bookCreateResponse.body.id;
...
describe('Loan Endpoints', () => {
  ...
  it('Should not allow the borrower to return a book', async () => {
    ...
    const response = await borrowerAgent
      .post(`/api/books/${bookId}/return`); // <-- Petición hecha por el prestatario
  ...
  // FIXME: For later, look for a solution for this two test
  it('Should allow the borrower to view their loans', async () => {
    ...
    const response = await borrowerAgent
      .get('/api/loans/borrowed'); // <-- Petición hecha por el prestatario
  ...
  it('Should allow the owner to view all loans', async () => {
    ...
    // Now, fetch all loans
    const response = await ownerAgent
      .get('/api/loans/lent'); // <-- Petición hecha por el dueño 
